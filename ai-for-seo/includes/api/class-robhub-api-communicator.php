...
class Ai4Seo_RobHubApiCommunicator {
	...
	// Seed the extensible generation registry here; later extensions do not inherit the built-in one-call cap.
	 ...
	 * endpoints are called once because RobHub owns their complete attempt budget.
	 *
	...
	function call( string $endpoint, array $parameters = array(), string $request_method = 'POST', bool $fallback_to_public_client_operation_credentials = false ) {
		...
		// Limit coordinated generation transport to one request so RobHub's three model attempts cannot be multiplied.
		$attempt = 0;

		$max_api_attempts_for_endpoint = $this->max_api_attempts;

		if ( in_array( $endpoint, self::COORDINATED_GENERATION_ENDPOINTS, true ) ) {
		...
		while ( $attempt < $max_api_attempts_for_endpoint ) {
			...
			$is_invalidate_auth_data_error = ( null !== $this_error_code && $this->is_auth_lock_error_code( $this_error_code ) );

			// Auth-locking errors must never retry, even if a future code list changes.
			$is_non_retriable  = $is_invalidate_auth_data_error || ( null !== $this_error_code && in_array( $this_error_code, $this->non_retriable_error_codes, true ) );
	...
	function prepare_call( $endpoint, $parameters, $request_method, $fallback_to_public_client_operation_credentials = false ) {
		...
		if ( ! $fallback_to_public_client_operation_credentials && ! $this->check_credentials() && 'client/get-free-account' !== $endpoint ) {
	 ...
	 */
	function check_raw_response( $raw_response, string $api_url, string $endpoint = '' ): array {
	 ...
	 */
	function get_api_error_from_response_body( string $raw_response_body ): array {
	 ...
	 */
	function get_http_status_error_response( int $http_status, string $api_url, string $raw_response_body, string $endpoint = '' ): array {
	...
	function try_handle_special_api_errors( $response ) {
		if ( isset( $response['code'] ) && is_numeric( $response['code'] ) ) {
			...
			if ( 371816823 === (int) $response['code'] ) {
	...
	function update_environmental_variable( string $environmental_variable_name, $new_environmental_variable_value ): bool {
		...
		if ( self::DEFAULT_ENVIRONMENTAL_VARIABLES[ $environmental_variable_name ] == $new_environmental_variable_value ) {
	...
	public function bulk_update_environmental_variables( array $environmental_variable_updates ): array {
		...
		foreach ( $environmental_variable_updates as $this_name => $this_value ) {
			...
			if ( self::DEFAULT_ENVIRONMENTAL_VARIABLES[ $this_name ] == $this_value ) {
