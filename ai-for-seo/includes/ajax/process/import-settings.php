...
if (isset($_REQUEST['ai4seo_import_settings_payload'])) {
    ...
    if ($ai4seo_import_settings_payload_encoding === 'base64_json') {
...
if (!isset($ai4seo_import_mode)) {
    $ai4seo_import_mode = isset($ai4seo_import_settings_request['ai4seo_import_mode']) && $ai4seo_import_settings_request['ai4seo_import_mode'] === 'execute' ? 'execute' : 'preview';
}
...
function ai4seo_parse_import_boolean_setting_value($raw_setting_value, bool &$is_valid): bool {
    ...
    if (is_int($raw_setting_value)) {
        if ($raw_setting_value === 1) {
            return true;
        }

        if ($raw_setting_value === 0) {
            return false;
        }
    ...
    if (is_string($raw_setting_value)) {
        ...
        if ($normalized_setting_value === 'true' || $normalized_setting_value === '1') {
            return true;
        }

        if ($normalized_setting_value === 'false' || $normalized_setting_value === '0') {
            return false;
        }
    }
...
if ($ai4seo_import_mode === 'preview') {
    ...
    if ($ai4seo_we_got_valid_settings) {
        if ($ai4seo_got_invalid_or_unknown_settings) {
            ...
            echo '</div>';
        } else if (!$ai4seo_we_got_valid_changes) {
            echo "<div class='ai4seo-medium-gap'>";
    ...
    foreach ($ai4seo_categorized_new_settings AS $ai4seo_this_category => $ai4seo_this_settings) {
        ...
        foreach ($ai4seo_this_settings AS $ai4seo_this_setting_name => $ai4seo_this_setting_new_value) {
            ...
            } else if ($ai4seo_this_setting_is_invalid) {
