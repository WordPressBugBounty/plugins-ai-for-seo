...
// A disabled attribute set is distinct from an empty Media Library, including when this template is used in a modal.
if ( ! ai4seo_get_active_attachment_attributes() ) {
	echo '<p>' . esc_html__( 'No active attachment attributes found.', 'ai-for-seo' ) . '</p>';
	return;
}
...
$ai4seo_attachment_coverage_read_succeeded                 = false;
...
if ( ! empty( $ai4seo_content_type_list_result['is_optimized'] ) ) {
	...
	if ( 'title' === $ai4seo_orderby ) {
		...
		$ai4seo_attachment_attributes_coverage_for_sorting         = ai4seo_read_and_analyse_attachment_attributes_coverage( $ai4seo_filtered_candidate_attachment_post_ids, $ai4seo_attachment_coverage_read_succeeded );
...
if ( 'seo_progress' === $ai4seo_orderby ) {
	...
	$ai4seo_attachment_attributes_coverage         = ai4seo_read_and_analyse_attachment_attributes_coverage( $ai4seo_current_attachment_post_ids, $ai4seo_attachment_coverage_read_succeeded );
...
foreach ( AI4SEO_ATTACHMENT_ATTRIBUTES_DETAILS as $ai4seo_field_identifier => $ai4seo_field_details ) {
	if ( in_array( $ai4seo_field_identifier, $ai4seo_active_attachment_attributes, true ) && isset( $ai4seo_field_details['name'] ) ) {
		$ai4seo_active_attachment_attribute_names[ $ai4seo_field_identifier ] = $ai4seo_field_details['name'];
	}
}
...
// Explain why automatic queuing is paused without leaking the notice into the related-media modal.
if ( ! $ai4seo_is_related_attachments_modal && empty( $ai4seo_bulk_generation_date_filter_state['is_valid'] ) ) {
...
foreach ( $ai4seo_all_attachment_posts as $ai4seo_this_attachment ) {
	...
	$ai4seo_is_excluded_by_new_or_existing_filter = ! empty( $ai4seo_bulk_generation_date_filter_state['is_valid'] )
		&& ! ai4seo_does_bulk_generation_date_filter_include_timestamp(
			...
			(int) $ai4seo_this_attachment_date_timestamp
		);
	...
	if ( $ai4seo_active_attachment_attributes ) {
		...
		$ai4seo_this_attachment_field_states = array();
		if ( $ai4seo_attachment_coverage_read_succeeded ) {
			foreach ( $ai4seo_attachment_attributes_coverage[ $ai4seo_this_post_attachment_id ] ?? array() as $ai4seo_attribute_identifier => $ai4seo_attribute_value ) {
				$ai4seo_this_attachment_field_states[ $ai4seo_attribute_identifier ] = $ai4seo_attribute_value ? 'filled' : 'missing';
			}
		}
		...
		echo ai4seo_get_seo_coverage_progress_bar_tag(
			...
			$ai4seo_active_attachment_attribute_names,
			$ai4seo_this_attachment_field_states
		);
		...
		if ( $ai4seo_is_attachment_post_waiting_to_get_queued ) {
			...
			echo "<div class='ai4seo-sub-info ai4seo-coverage-status'>";
			echo esc_html__( 'All active media attributes are filled. SEO Autopilot is leaving existing values unchanged under your overwrite settings.', 'ai-for-seo' );
			echo '</div>';
		}
	...
	if ( $ai4seo_active_attachment_attributes ) {
		ai4seo_echo_wp_kses(
			ai4seo_get_edit_attachment_attributes_button(
				$ai4seo_this_post_attachment_id,
				$ai4seo_current_attachment_post_ids,
				$ai4seo_media_label_singular,
				$ai4seo_attachment_coverage_read_succeeded && $ai4seo_this_attachment_attribute_coverage_percentage < 100
			)
		);
	}
			echo '</td>';
