...
if ( ! function_exists( 'ai4seo_read_content_type_post_list_page_ids' ) ) {
	...
	function ai4seo_read_content_type_post_list_page_ids(
		...
		$sort_args               = ai4seo_normalize_content_type_sort_args( $orderby, $order );
		$is_ascending            = ( 'asc' === $sort_args['order'] );

		while ( $visible_post_id_count < $limit ) {
			$values    = $sql_parts['values'];
			$where_sql = $sql_parts['where_sql'];

			// Seek from the last raw ID so earlier blocks are not skipped again by SQL.
			if ( null !== $last_post_id ) {
				$where_sql .= ( '' === $where_sql ? ' WHERE ' : ' AND ' ) . ( $is_ascending ? 'ID > %d' : 'ID < %d' );
				$values[]   = $last_post_id;
			}

			$sql      = "SELECT ID FROM {$wpdb->posts}{$where_sql}{$orderby_sql} LIMIT %d";
			$values[] = $batch_size;
			...
			// Advance even when the whole raw block is hidden; reject stalled or invalid cursors.
			$next_post_id = filter_var(
				end( $batch_post_ids ),
				FILTER_VALIDATE_INT,
			...
			if ( false === $next_post_id
				|| ( null !== $last_post_id && ( $is_ascending ? $next_post_id <= $last_post_id : $next_post_id >= $last_post_id ) )
			) {
				ai4seo_debug_message( 984321715, 'Invalid or non-advancing content list cursor.' );
				return array();
			}
