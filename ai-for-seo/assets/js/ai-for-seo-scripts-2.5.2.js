...
// Keep clean form snapshots and their coalesced refresh state under stable jQuery data keys.
const ai4seo_unsaved_changes_data_attribute = 'ai4seo-unsaved-changes';
const ai4seo_unsaved_changes_baseline_data_attribute = 'ai4seo-unsaved-changes-baseline';
const ai4seo_unsaved_changes_refresh_pending_data_attribute = 'ai4seo-unsaved-changes-refresh-pending';
...
// Coalesce generation-importance refreshes by stable DOM root and processing context within one frame.
const ai4seo_pending_generation_button_primary_state_refreshes = new Map();
let ai4seo_generation_button_primary_state_refresh_request_id = null;
...
function ai4seo_init_shared_primary_asset_elements(scope = null) {
	...
	ai4seo_init_bulk_generation_queue_action_forms( scope );
...
function ai4seo_update_container_submit_buttons_state($container, is_active, is_initial = false) {
	...
	$buttons.each(
		...
		ai4seo_set_disabled_state_respecting_temporary_lock( $button, !is_active );
...
/**
 * Collect a stable snapshot of the named persistent inputs in one tracked container.
 *
 * @param {*} $container
 * @returns {Object|null}
 */
function ai4seo_get_unsaved_changes_snapshot($container) {
	$container = ai4seo_get_unsaved_changes_container( $container );

	if (!ai4seo_exists_$( $container )) {
		console.warn( ai4seo_get_plugin_name() + ': container missing in ai4seo_get_unsaved_changes_snapshot() — cannot collect tracked values.' );
		return null;
	}

	const input_values = ai4seo_get_all_input_values_in_container( $container, false );
	...
	Object.keys( input_values ).sort().forEach(
		function (input_name) {
			const input_value = input_values[input_name];

			// Checkbox groups are sets for dirty-state purposes, so DOM order must not create a false change.
			snapshot[input_name] = Array.isArray( input_value ) ? [...input_value].sort() : input_value;
		}
	);
...
/**
 * Convert a tracked-input snapshot into a deterministic comparison value.
 *
 * @param {Object} snapshot
 * @returns {string}
 */
function ai4seo_serialize_unsaved_changes_snapshot(snapshot) {
	if (!snapshot || typeof snapshot !== 'object') {
		return '';
	}

	return JSON.stringify(
		Object.keys( snapshot ).sort().map(
			function (input_name) {
				return [input_name, snapshot[input_name]];
			}
		)
	);
}
...
/**
 * Store the current tracked values as the clean baseline for one container.
 *
 * @param {*} $container
 * @param {boolean} [preserve_existing_baseline_values=false]
 * @returns {boolean}
 */
function ai4seo_store_unsaved_changes_baseline($container, preserve_existing_baseline_values = false) {
	$container = ai4seo_get_unsaved_changes_container( $container );

	if (!ai4seo_exists_$( $container )) {
		console.warn( ai4seo_get_plugin_name() + ': container missing in ai4seo_store_unsaved_changes_baseline() — cannot store tracked values.' );
		return false;
	}

	const current_snapshot = ai4seo_get_unsaved_changes_snapshot( $container );

	if (current_snapshot === null) {
		return false;
	}

	let baseline_snapshot = current_snapshot;
	const existing_baseline = $container.data( ai4seo_unsaved_changes_baseline_data_attribute );

	// Reinitialization retains known clean values, adopts new inputs, and drops inputs removed from the DOM.
	if (preserve_existing_baseline_values && existing_baseline && typeof existing_baseline === 'object') {
		baseline_snapshot = {};

		Object.keys( current_snapshot ).forEach(
			function (input_name) {
				baseline_snapshot[input_name] = Object.prototype.hasOwnProperty.call( existing_baseline, input_name )
					? existing_baseline[input_name]
					: current_snapshot[input_name];
			}
		);
	}

	$container.data( ai4seo_unsaved_changes_baseline_data_attribute, baseline_snapshot );
...
/**
 * Recompute one container's dirty state from its current values and stored baseline.
 *
 * @param {*} $container
 * @returns {boolean}
 */
function ai4seo_refresh_unsaved_changes_state($container) {
	$container = ai4seo_get_unsaved_changes_container( $container );

	if (!ai4seo_exists_$( $container )) {
		console.warn( ai4seo_get_plugin_name() + ': container missing in ai4seo_refresh_unsaved_changes_state() — cannot refresh tracked state.' );
		return false;
	}

	const baseline_snapshot = $container.data( ai4seo_unsaved_changes_baseline_data_attribute );
	const current_snapshot = ai4seo_get_unsaved_changes_snapshot( $container );

	// Preserve the existing fail-safe behavior if a baseline or current value set cannot be resolved.
	const has_unsaved_changes = !baseline_snapshot
		|| typeof baseline_snapshot !== 'object'
		|| current_snapshot === null
		|| ai4seo_serialize_unsaved_changes_snapshot( current_snapshot ) !== ai4seo_serialize_unsaved_changes_snapshot( baseline_snapshot );

	ai4seo_set_unsaved_changes_state( $container, has_unsaved_changes );
...
/**
 * Schedule at most one dirty-state comparison per container and rendering frame.
 *
 * @param {*} $container
 */
function ai4seo_schedule_unsaved_changes_state_refresh($container) {
	$container = ai4seo_get_unsaved_changes_container( $container );
	...
	if ($container.data( ai4seo_unsaved_changes_refresh_pending_data_attribute ) === true) {
		return;
	}

	// Mark the container before scheduling so rapid input events share one comparison pass.
	$container.data( ai4seo_unsaved_changes_refresh_pending_data_attribute, true );
	ai4seo_set_unsaved_changes_state( $container, true );

	ai4seo_schedule_next_animation_frame(
		function () {
			$container.removeData( ai4seo_unsaved_changes_refresh_pending_data_attribute );

			const container_element = $container.get( 0 );

			// Detached AJAX markup must not update controls that no longer belong to the document.
			if (!container_element || container_element.isConnected === false) {
				return;
			}

			ai4seo_refresh_unsaved_changes_state( $container );
		}
	);
}
...
function ai4seo_remove_unsaved_changes_attribute(element) {
	...
	// Removing tracking also discards comparison state that cannot apply to a future initialization.
	$container.removeAttr( data_attribute_name );
	$container.removeData( ai4seo_unsaved_changes_data_attribute );
	$container.removeData( ai4seo_unsaved_changes_baseline_data_attribute );
	$container.removeData( ai4seo_unsaved_changes_refresh_pending_data_attribute );
...
function ai4seo_init_unsaved_changes_warnings(scope = null) {
	...
	$unsaved_changes_containers.each(
		...
		// Preserve dirty values across repeated initialization while adding newly inserted clean inputs to their baseline.
		const has_unsaved_changes_attribute = typeof $container.attr( 'data-' + ai4seo_unsaved_changes_data_attribute ) !== 'undefined';
		const has_unsaved_changes_baseline = typeof $container.data( ai4seo_unsaved_changes_baseline_data_attribute ) === 'object';
		const has_pending_refresh = $container.data( ai4seo_unsaved_changes_refresh_pending_data_attribute ) === true;
		const has_unsaved_changes = has_unsaved_changes_attribute && ai4seo_container_has_unsaved_changes( $container );

		if (!has_unsaved_changes_baseline || (!has_unsaved_changes && !has_pending_refresh)) {
			ai4seo_store_unsaved_changes_baseline( $container );
		} else {
			ai4seo_store_unsaved_changes_baseline( $container, true );
		}

		if (!has_unsaved_changes_attribute) {
			ai4seo_set_unsaved_changes_state( $container, false, true );
		} else {
			// Reconcile the cached flag after the baseline drops controls removed since the previous initialization.
			ai4seo_refresh_unsaved_changes_state( $container );
		}
		...
		$inputs.on(
			...
			function () {
				ai4seo_schedule_unsaved_changes_state_refresh( this );
			}
		);
        }
    );
	...
	const bridge_request = (async function () {
		...
		try {
			if (!rest_url || !nonce || !Number.isInteger( post_id ) || post_id <= 0 || typeof window.fetch !== 'function') {
				// Reuse the shared catch path for validation failures and WeakMap cleanup.
			...
			if (!bridge_response.ok) {
				// Reuse the shared catch path for non-successful HTTP responses.
				// noinspection ExceptionCaughtLocallyJS
...
function ai4seo_init_generate_buttons(scope = null, processing_context = null) {
			...
			$this_generate_data_for_inputs.each(
				function () {
					...
					$this_generate_data_for_input
						.off( 'input.ai4seo-generate-button-injection change.ai4seo-generate-button-injection' )
						.on(
							'input.ai4seo-generate-button-injection change.ai4seo-generate-button-injection',
	...
	// Apply individual fallback importance after every matching field has its generation control.
	ai4seo_schedule_generation_button_primary_state_refresh( scope, processing_context );
}
...
/**
 * Keep a custom bulk action's Apply button aligned with its two required inputs.
 *
 * @param {HTMLElement|jQuery} form
 */
function ai4seo_update_bulk_generation_queue_action_submit_button_state(form) {
	const $form = ai4seo_normalize_$( form );
	...
	const $select = $form.find( '.ai4seo-bulk-generation-queue-action-select' ).first();
	const $submit_button = $form.find( '.ai4seo-bulk-generation-queue-action-submit' ).first();
	const checkbox_name = String( $form.find( '.ai4seo-bulk-generation-queue-checkbox-name' ).val() || '' );
	const $checkbox_scope = $form.closest( '.ai4seo-related-attachments-modal-content' ).first();

	// Use the submission helper's selection scope so button state and submitted IDs cannot diverge.
	const has_selected_action = ai4seo_exists_$( $select ) && String( $select.val() || '' ) !== '';
	const has_selected_entry = checkbox_name !== '' && ai4seo_get_selected_bulk_generation_queue_post_ids( checkbox_name, $checkbox_scope ).length > 0;
	const can_submit = has_selected_action && has_selected_entry;

	if (!ai4seo_exists_$( $submit_button )) {
		return;
	}

	ai4seo_set_disabled_state_respecting_temporary_lock( $submit_button, !can_submit );
	$submit_button.toggleClass( 'ai4seo-inactive-button', !can_submit );
}
 ...
 * @param {jQuery|Node|Window|null} scope Optional root for dynamically inserted list controls.
 */
function ai4seo_init_bulk_generation_queue_action_forms(scope = null) {
	const $bulk_generation_queue_action_forms = ai4seo_get_elements_in_scope_$( '.ai4seo-bulk-generation-queue-action-form', scope );
	...
	// Each form owns its action control while its row checkboxes may live in the surrounding list surface.
	$bulk_generation_queue_action_forms.each(
		function () {
			const $form = ai4seo_normalize_$( this );
			const $select = $form.find( '.ai4seo-bulk-generation-queue-action-select' ).first();
			const $submit_button = $form.find( '.ai4seo-bulk-generation-queue-action-submit' ).first();
			const checkbox_name = String( $form.find( '.ai4seo-bulk-generation-queue-checkbox-name' ).val() || '' );
			const checkbox_selector = "input[type='checkbox'][name='" + checkbox_name + "[]']:not(:disabled)";
			const $checkbox_scope = $form.closest( '.ai4seo-related-attachments-modal-content' ).first();
			const $entry_checkboxes = checkbox_name === ''
				? jQuery()
				: (ai4seo_exists_$( $checkbox_scope ) ? $checkbox_scope.find( checkbox_selector ) : ai4seo_normalize_$( checkbox_selector ));

			// Reuse one closure so action and row changes cannot drift into separate state paths.
			const ai4seo_refresh_submit_button_state = function () {
				ai4seo_update_bulk_generation_queue_action_submit_button_state( $form );
			};

			// Bind form submit once per init pass; modal/page refreshes call this function repeatedly.
			$form.off( 'submit.ai4seo-bulk-generation-queue' );
			$form.on(
			...
			$submit_button.off( 'click.ai4seo-bulk-generation-queue' );
			$submit_button.on(
				'click.ai4seo-bulk-generation-queue',
				function (event) {
					event.preventDefault();
					ai4seo_submit_bulk_generation_queue_action_form( $form );
				}
			);

			// Either prerequisite can change independently, so both handlers reuse the same state calculation.
			$select.off( 'change.ai4seo-bulk-generation-queue-action-state' );
			$select.on(
				'change.ai4seo-bulk-generation-queue-action-state',
				ai4seo_refresh_submit_button_state
			);

			$entry_checkboxes.off( 'change.ai4seo-bulk-generation-queue-action-state' );
			$entry_checkboxes.on(
				'change.ai4seo-bulk-generation-queue-action-state',
				ai4seo_refresh_submit_button_state
			);

			// Browser-restored selections need the same initial state as later user changes.
			ai4seo_refresh_submit_button_state();
		}
	);
}
...
function ai4seo_submit_bulk_generation_queue_action_form(bulk_generation_queue_action_form) {
	...
	ai4seo_perform_ajax_call(
		...
		.finally(
			() => {
				// Re-evaluate prerequisites after shared lock cleanup restores the page controls.
				ai4seo_remove_loading_html_from_element( $submit_button );
				ai4seo_unlock_and_enable_lockable_input_fields();
				ai4seo_update_bulk_generation_queue_action_submit_button_state( $bulk_generation_queue_action_form );
			}
		);
...
/**
 * Enable the destructive reset action only when its owning surface has a selected entry type.
 *
 * @param {string} checkbox_selector Reset checkbox selector.
 * @param {string} submit_button_selector Reset submit button selector.
 * @param {jQuery|Node|Window|null} scope Optional form or modal boundary.
 */
function ai4seo_update_generated_data_reset_submit_button_state(checkbox_selector, submit_button_selector, scope = null) {
	// Resolve both sides within one modal so stacked confirmations cannot share selection state.
	const $post_type_checkboxes = ai4seo_normalize_$( checkbox_selector, scope );
	const $submit_button = ai4seo_normalize_$( submit_button_selector, scope );

	if (!ai4seo_exists_$( $submit_button )) {
		return;
	}

	const has_selected_post_type = ai4seo_exists_$( $post_type_checkboxes ) && $post_type_checkboxes.filter( ':checked' ).length > 0;

	$submit_button.prop( 'disabled', !has_selected_post_type );
	$submit_button.toggleClass( 'ai4seo-inactive-button', !has_selected_post_type );
}

// =========================================================================================== \\

/**
 * Bind one generated-data reset action to the selection state inside its owning surface.
 *
 * @param {string} checkbox_selector Reset checkbox selector.
 * @param {string} submit_button_selector Reset submit button selector.
 * @param {jQuery|Node|Window|null} scope Optional form or modal boundary.
 */
function ai4seo_init_generated_data_reset_submit_button_state(checkbox_selector, submit_button_selector, scope = null) {
	// Rebind only the supplied surface while still applying its server-rendered initial selection.
	const $post_type_checkboxes = ai4seo_normalize_$( checkbox_selector, scope );
	const $submit_button = ai4seo_normalize_$( submit_button_selector, scope );
	...
	if (ai4seo_exists_$( $post_type_checkboxes )) {
		$post_type_checkboxes.off( 'change.ai4seo-generated-data-reset-submit-button' );
		$post_type_checkboxes.on(
			'change.ai4seo-generated-data-reset-submit-button',
			function () {
				ai4seo_update_generated_data_reset_submit_button_state( checkbox_selector, submit_button_selector, scope );
			}
		);
	}
...
function ai4seo_confirm_autopilot_remove_generated_data() {
	...
	const $notification_modal = ai4seo_open_notification_modal(
		...
		"<button type='button' class='ai4seo-button ai4seo-primary-button' onclick='ai4seo_close_modal_by_child(this);'>" + wp.i18n.__( 'Abort', 'ai-for-seo' ) + "</button><button type='button' class='ai4seo-button ai4seo-secondary-button ai4seo-inactive-button ai4seo-autopilot-reset-generated-data-submit-button' onclick='ai4seo_remove_generated_data_via_autopilot(this);' disabled>" + wp.i18n.__( 'Remove selected data', 'ai-for-seo' ) + '</button>',
	...
	// Initialize checkbox fan-out before the warning and submit state derived from those checkboxes.
	...
	ai4seo_init_generated_data_reset_submit_button_state(
		'.ai4seo-autopilot-reset-generated-data-post-type-checkbox',
		'.ai4seo-autopilot-reset-generated-data-submit-button',
		$notification_modal
	);
}
	...
	$scope.each(
		function () {
			...
			try {
				...
				// Ignore candidates that cannot be enumerated by jQuery and continue with the remaining scope.
			}
		}
	);
...
function ai4seo_is_yoast_element($yoast_candidate) {
	...
	return ai4seo_exists_$($yoast_input_editor);
}
...
function ai4seo_generate_with_ai($generate_button, ajax_action, generate_data_for_input_instructions = [], post_id = false, overwrite_data = true, try_read_page_content_via_js = false) {
	...
	// Remove the importance cue while the requested fields are actively being generated.
	$generate_button.removeClass( 'ai4seo-primary-button' );
	jQuery.each(
		affected_generate_buttons,
		function (_, $affected_generate_button) {
			ai4seo_normalize_$( $affected_generate_button ).removeClass( 'ai4seo-primary-button' );
		}
	);
...
function ai4seo_get_normalized_generation_fields(generate_data_for_input_instructions, overwrite_data = true, scope = null, post_id = false) {
	...
	jQuery.each(
		...
		function (this_generation_field_identifier) {
		jQuery.each(
            ai4seo_generate_data_for_inputs,
...
function ai4seo_check_response(response, additional_error_list = {}, show_generic_error = true, add_contact_us_link = true) {
	...
	if (show_generic_error) {
		...
		if (add_contact_us_link) {
			...
			// The sprintf placeholder is replaced with the trusted support URL at runtime.
...
/**
 * Resolves the generation field identifier shared by metadata and attachment registry entries.
 *
 * @param {Object} generate_data_for_input_details Generation registry entry.
 * @return {string} Metadata or attachment field identifier.
 */
function ai4seo_get_generation_field_identifier(generate_data_for_input_details = {}) {
	if (!generate_data_for_input_details || typeof generate_data_for_input_details !== 'object') {
		return '';
	}

	const generation_field_identifier = generate_data_for_input_details.metadata_identifier
		|| generate_data_for_input_details.attachment_attributes_identifier
		|| '';

	return typeof generation_field_identifier === 'string' ? generation_field_identifier : '';
}

// =========================================================================================== \\

/**
 * Resolves the active field identifiers through the authoritative reader for one generation context.
 *
 * @param {string} processing_context Metadata or attachment-attributes context.
 * @return {Array|undefined} Active generation field identifiers.
 */
function ai4seo_get_active_generation_field_identifiers(processing_context) {
	if (processing_context === 'metadata') {
		return ai4seo_get_active_meta_tags();
	}

	if (processing_context === 'attachment-attributes') {
		return ai4seo_get_active_attachment_attributes();
	}
...
/**
 * Schedule one generation-button importance refresh per editor/context for the next frame.
 *
 * @param {jQuery|HTMLElement|null} scope Optional editor instance.
 * @param {string|null} processing_context Optional metadata or attachment-attributes filter.
 */
function ai4seo_schedule_generation_button_primary_state_refresh(scope = null, processing_context = null) {
	const resolved_scope = ai4seo_resolve_html_elements_init_scope( scope );
	const pending_processing_contexts = ai4seo_pending_generation_button_primary_state_refreshes.get( resolved_scope ) || new Set();

	ai4seo_pending_generation_button_primary_state_refreshes.set( resolved_scope, pending_processing_contexts );

	// An all-context request subsumes narrower work, while distinct specific contexts remain independent.
	if (processing_context === null) {
		pending_processing_contexts.clear();
		pending_processing_contexts.add( null );
	} else if (!pending_processing_contexts.has( null )) {
		pending_processing_contexts.add( processing_context );
	}

	if (ai4seo_generation_button_primary_state_refresh_request_id !== null) {
		return;
	}

	ai4seo_generation_button_primary_state_refresh_request_id = ai4seo_schedule_next_animation_frame(
		function () {
			const pending_refreshes = new Map( ai4seo_pending_generation_button_primary_state_refreshes );

			// Release every scope before refreshing so remounted editors cannot be retained across frames.
			ai4seo_pending_generation_button_primary_state_refreshes.clear();
			ai4seo_generation_button_primary_state_refresh_request_id = null;

			for (const [refresh_scope, refresh_processing_contexts] of pending_refreshes) {
				for (const refresh_processing_context of refresh_processing_contexts) {
					// Keep one malformed or unmounted editor from dropping unrelated queued refreshes.
					try {
						ai4seo_refresh_generation_button_primary_states( refresh_scope, refresh_processing_context );
					} catch (error) {
						console.error(
							ai4seo_get_plugin_name() + ': Failed to refresh generation-button primary state for one queued editor/context — continuing remaining refreshes.',
							error
						);
					}
				}
...
/**
 * Keeps generation-button importance aligned with empty fields and actual Generate All coverage.
 *
 * @param {jQuery|HTMLElement|null} scope Optional editor instance.
 * @param {string|null} processing_context Optional metadata or attachment-attributes filter.
 */
function ai4seo_refresh_generation_button_primary_states(scope = null, processing_context = null) {
	// Cache one normalized empty-field snapshot per editor/context and one parsed coverage set per button.
	const generation_container_states = new Map();
	const generate_all_button_field_identifiers = new WeakMap();

	// Keep coverage parsing local to this refresh so rebuilt controls cannot retain stale field identifiers.
	const ai4seo_get_generate_all_button_field_identifiers = function (generate_all_button) {
		if (generate_all_button_field_identifiers.has( generate_all_button )) {
			return generate_all_button_field_identifiers.get( generate_all_button );
		}

		const $generate_all_button = ai4seo_normalize_$( generate_all_button );
		const field_identifiers = new Set(
			String( $generate_all_button.attr( 'data-ai4seo-generation-fields' ) || '' )
				.split( ',' )
				.map( field_identifier => field_identifier.trim() )
				.filter( Boolean )
		);

		generate_all_button_field_identifiers.set( generate_all_button, field_identifiers );
		return field_identifiers;
	};

	// Reuse the parsed coverage rule for both empty-fields actions and the sole-action fallback.
	const ai4seo_generate_all_button_covers_empty_field = function (generate_all_button, empty_field_identifiers) {
		return [...ai4seo_get_generate_all_button_field_identifiers( generate_all_button )].some(
			field_identifier => empty_field_identifiers.has( field_identifier )
		);
	};

	// Normalize each closest editor container once so duplicate selectors share authoritative editor state.
	const ai4seo_get_generation_container_state = function ($reference_element, this_processing_context) {
		$reference_element = ai4seo_normalize_$( $reference_element );

		if (!ai4seo_exists_$( $reference_element ) || !this_processing_context) {
			return null;
		}

		const $closest_container = ai4seo_get_closest_container_$( $reference_element );
		const closest_container_element = $closest_container.get( 0 );

		if (!closest_container_element) {
			return null;
		}

		let processing_context_states = generation_container_states.get( closest_container_element );

		if (!processing_context_states) {
			processing_context_states = new Map();
			generation_container_states.set( closest_container_element, processing_context_states );
		}

		if (processing_context_states.has( this_processing_context )) {
			return processing_context_states.get( this_processing_context );
		}

		const active_generation_field_identifiers = ai4seo_get_active_generation_field_identifiers( this_processing_context ) || [];

		const post_id = ai4seo_get_post_id( this_processing_context, $closest_container );
		const empty_generation_fields = ai4seo_get_normalized_generation_fields(
			active_generation_field_identifiers,
			false,
			$closest_container,
			post_id
		);
		// Recheck closest-container identity so nested or stacked editors never provide cross-editor coverage.
		const $generate_all_buttons = ai4seo_get_elements_in_scope_$( '.ai4seo-generate-all-button', $closest_container )
			.filter(
				function () {
					const $this_generate_all_button = ai4seo_normalize_$( this );

					return $this_generate_all_button.attr( 'data-ai4seo-processing-context' ) === this_processing_context
						&& ai4seo_get_closest_container_$( $this_generate_all_button ).get( 0 ) === closest_container_element;
				}
			);
		const generation_container_state = {
			'$generate_all_buttons': $generate_all_buttons,
			'empty_field_identifiers': new Set( Object.keys( empty_generation_fields || {} ) ),
		};

		processing_context_states.set( this_processing_context, generation_container_state );
		return generation_container_state;
	};

	// Seed states from Generate All controls so proxy-only and lazily mounted editors remain covered.
	ai4seo_get_elements_in_scope_$( '.ai4seo-generate-all-button', scope ).each(
		function () {
			const $generate_all_button = ai4seo_normalize_$( this );
			const this_processing_context = $generate_all_button.attr( 'data-ai4seo-processing-context' ) || '';

			if (processing_context !== null && this_processing_context !== processing_context) {
				return;
			}

			ai4seo_get_generation_container_state( $generate_all_button, this_processing_context );
		}
	);

	// Individual buttons are primary only for an empty field without matching Generate All coverage.
	jQuery.each(
		ai4seo_generate_data_for_inputs,
		function (generate_data_for_input_selector, generate_data_for_input_details) {
			const this_processing_context = generate_data_for_input_details['processing-context'] || '';

			if (processing_context !== null && this_processing_context !== processing_context) {
				return;
			}

			if (generate_data_for_input_details.add_generate_button === false
				|| !ai4seo_should_render_generate_button_for_selector( generate_data_for_input_selector, generate_data_for_input_details )) {
				return;
			}

			const generation_field_identifier = ai4seo_get_generation_field_identifier( generate_data_for_input_details );

			if (!generation_field_identifier) {
				return;
			}

			ai4seo_get_elements_in_scope_$( generate_data_for_input_selector, scope ).each(
				function () {
					const $generate_data_for_input = ai4seo_normalize_$( this );
					const $generate_button = ai4seo_try_find_generate_button_by_input_$( $generate_data_for_input );

					if (!ai4seo_exists_$( $generate_button ) || ai4seo_is_generate_button_busy( $generate_button )) {
						return;
					}

					const generation_container_state = ai4seo_get_generation_container_state( $generate_data_for_input, this_processing_context );

					if (!generation_container_state) {
						$generate_button.removeClass( 'ai4seo-primary-button' );
						return;
					}

					const is_generation_field_empty = generation_container_state.empty_field_identifiers.has( generation_field_identifier );
					const has_generate_all_coverage = generation_container_state.$generate_all_buttons.toArray().some(
						generate_all_button => ai4seo_get_generate_all_button_field_identifiers( generate_all_button ).has( generation_field_identifier )
					);

					$generate_button.toggleClass( 'ai4seo-primary-button', is_generation_field_empty && !has_generate_all_coverage );
				}
			);
		}
	);

	// Prefer the non-overwriting empty-fields action, with the sole-action fallback for all-empty editors.
	for (const processing_context_states of generation_container_states.values()) {
		for (const generation_container_state of processing_context_states.values()) {
			generation_container_state.$generate_all_buttons
				.closest( '.ai4seo-generate-all-button-wrapper' )
				.each(
					function () {
						const $generate_all_buttons = ai4seo_normalize_$( this )
							.find( '.ai4seo-generate-all-button' )
							.filter(
								function () {
									return generation_container_state.$generate_all_buttons.is( this );
								}
							);
						const $idle_generate_all_buttons = $generate_all_buttons.filter(
							function () {
								return !ai4seo_is_generate_button_busy( ai4seo_normalize_$( this ) );
							}
						);

						$idle_generate_all_buttons.removeClass( 'ai4seo-primary-button' );

						if (generation_container_state.empty_field_identifiers.size === 0) {
							return;
						}

						const $generate_empty_fields_buttons = $idle_generate_all_buttons
							.filter( '[data-ai4seo-overwrite-existing-content="0"]' )
							.filter(
								function () {
									return ai4seo_generate_all_button_covers_empty_field( this, generation_container_state.empty_field_identifiers );
								}
							);

						if (ai4seo_exists_$( $generate_empty_fields_buttons )) {
							$generate_empty_fields_buttons.addClass( 'ai4seo-primary-button' );
							return;
						}

						if ($generate_all_buttons.length === 1 && $idle_generate_all_buttons.length === 1) {
							const sole_button_covers_empty_field = ai4seo_generate_all_button_covers_empty_field(
								$idle_generate_all_buttons.get( 0 ),
								generation_container_state.empty_field_identifiers
							);

							$idle_generate_all_buttons.toggleClass( 'ai4seo-primary-button', sole_button_covers_empty_field );
						}
					}
				);
		}
...
function ai4seo_init_generate_all_buttons(scope = null, processing_context = null) {
	...
	// Recompute coverage after wrappers are rebuilt because Generate All controls are replaced wholesale.
	ai4seo_schedule_generation_button_primary_state_refresh( scope, processing_context );
}
...
function ai4seo_add_generate_all_buttons(processing_context, $generate_all_buttons_container) {
	...
	// Resolve generation fields through the same context reader used by primary-state normalization.
	const active_generation_field_identifiers = ai4seo_get_active_generation_field_identifiers( processing_context );
	...
	jQuery.each(
		...
		let generation_field_identifiers = [];
		const button_icon_html = ai4seo_get_sooz_logo_svg_tag( 'sooz-oo' );

		if (processing_context === 'metadata') {
			const normalized_generation_fields = ai4seo_get_normalized_generation_fields( active_generation_field_identifiers, overwrite_existing_content, $closest_container, post_id );
			...
			generation_field_identifiers = Object.keys( normalized_generation_fields );
			const human_readable_generated_field_identifiers = ai4seo_get_human_readable_generation_field_names( generation_field_identifiers );
		...
		// Expose context, coverage, and overwrite behavior as the runtime contract for primary-state detection.
		const generate_all_button_data_attributes =
			" data-ai4seo-processing-context='" + processing_context + "'" +
			" data-ai4seo-generation-fields='" + generation_field_identifiers.join( ',' ) + "'" +
			" data-ai4seo-overwrite-existing-content='" + (overwrite_existing_content ? '1' : '0') + "'";

		// Keep the semantic data contract distinct from the button's presentation markup.
		button_html += "<button type='button' onclick='" + onclick + "' title='" + button_title + "'" +
			generate_all_button_data_attributes +
			" class='ai4seo-button ai4seo-big-button ai4seo-generate-all-button ai4seo-lockable'>" +
			"<span class='ai4seo-generation-button-icon'>" + button_icon_html + '</span>' +
			"<span class='ai4seo-generation-button-label'>" + button_label + '</span>' +
			credits_badge_html +
			'</button>';
        }
    );
...
function ai4seo_set_modal_content(modal_id, content_html) {
	...
	// Normalize markup and DOM content to a jQuery element.
	let $content = ai4seo_normalize_$( content_html );
...
/**
 * Collect named persistent input values from one form container.
 *
 * @param {*} $form_container
 * @param {boolean} [show_empty_input_notification=true] Whether an empty user-facing save should open its warning modal.
 * @return {Object|boolean}
 */
function ai4seo_get_all_input_values_in_container($form_container, show_empty_input_notification = true) {
	...
	if (Object.keys( input_values ).length === 0) {
		// Background comparison scans suppress the save-flow notification but retain the established false result.
		if (show_empty_input_notification) {
...
function ai4seo_validate_metadata_editor_inputs(input_values) {
	...
	return ai4seo_validate_custom_instruction_input_values( input_values, 3116141025 );
}
...
function ai4seo_validate_attachment_attributes_editor_inputs(input_values) {
	...
	return ai4seo_validate_custom_instruction_input_values( input_values, 3216141025 );
}
...
function ai4seo_save_anything($submit_button, validation_function, success_function, error_function) {
	...
	ai4seo_perform_ajax_call( 'ai4seo_save_anything', ajax_data )
		.then(
			...
			window.scrollTo( 0, 0 );
			// Treat the values accepted by the server as the new clean comparison baseline.
			ai4seo_store_unsaved_changes_baseline( $closest_form_container );
...
function ai4seo_set_license_key_visibility(show_actual_license_key) {
	// Synchronize saved-key holders or an editable password field with the shared toggle state.
	const $license_key_input_wrapper = ai4seo_normalize_$( '.ai4seo-license-key-input-wrapper' );
	...
	const is_license_key_entry_mode = ai4seo_exists_$( $license_key_input_wrapper ) && $license_key_input_wrapper.hasClass( 'ai4seo-license-key-entry-mode' );

	// Editable entry mode masks the real input; saved-key mode swaps its actual and visual holders.
	if (is_license_key_entry_mode) {
		if (!ai4seo_exists_$( $license_key_input )) {
			return;
		}

		// Replacement mode may retain the saved-key holders until the account page reloads.
		if (ai4seo_exists_$( $visual_license_key_holder )) {
			$visual_license_key_holder.hide().addClass( 'ai4seo-display-none' ).css( 'display', '' );
		}

		if (ai4seo_exists_$( $actual_license_key_holder )) {
			$actual_license_key_holder.removeClass( 'ai4seo-display-none' ).show().css( 'display', '' );
		}

		$license_key_input.attr( 'type', show_actual_license_key ? 'text' : 'password' );
	} else {
...
function ai4seo_enable_license_key_entry_mode(license_key_input_wrapper) {
	// Convert a saved-key wrapper into a masked editable field for reconnecting another license.
	const $license_key_input_wrapper = ai4seo_normalize_$( license_key_input_wrapper );

	if (!ai4seo_exists_$( $license_key_input_wrapper )) {
		return false;
	}

	const $license_key_input = ai4seo_normalize_$( $license_key_input_wrapper.find( "input[name='ai4seo_api_password']" ) );
	const $license_key_toggle = ai4seo_normalize_$( $license_key_input_wrapper.find( '.ai4seo-license-key-toggle' ) );

	if (!ai4seo_exists_$( $license_key_input )) {
		return false;
	}

	$license_key_input_wrapper.addClass( 'ai4seo-license-key-entry-mode' );

	// Treat replacement credentials as newly entered secrets so password managers do not autofill saved admin credentials.
	$license_key_input.attr( 'autocomplete', 'new-password' );

	if (ai4seo_exists_$( $license_key_toggle )) {
		const license_key_input_id = String( $license_key_input.attr( 'id' ) || '' );

		if (license_key_input_id !== '') {
			$license_key_toggle.attr( 'aria-controls', license_key_input_id );
		}

		$license_key_toggle.show();
	}

	ai4seo_set_license_key_visibility( false );
	return true;
}
...
function ai4seo_init_license_key_toggle() {
	// Bind the shared account-page toggle for both saved credentials and editable replacement keys.
	const $license_key_toggle = ai4seo_normalize_$( '.ai4seo-license-key-toggle' );
...
function ai4seo_open_lost_key_modal() {
	const modal_headline = wp.i18n.__( 'Recover license data', 'ai-for-seo' );
	const contact_link = "<a href='" + ai4seo_official_contact_url + "' target='_blank' rel='noopener noreferrer'>" + wp.i18n.__( 'contact support', 'ai-for-seo' ) + '</a>';
	let modal_content = '<p>' + wp.i18n.__( 'For security, license credentials are delivered only to the exact verified Stripe checkout email when a purchase or password rotation completes. Check that inbox and its spam folder.', 'ai-for-seo' ) + '</p>';
	/* translators: %s: Contact-support link. */
	modal_content += '<p>' + wp.i18n.sprintf( wp.i18n.__( 'If the credentials are not available there, %s with the Stripe invoice or checkout details so the purchase can be verified safely.', 'ai-for-seo' ), contact_link ) + '</p>';

	const modal_footer = "<button type='button' class='ai4seo-button ai4seo-primary-button' onclick='ai4seo_close_modal_by_child(this);'>" + wp.i18n.__( 'Close', 'ai-for-seo' ) + '</button>';
...
function ai4seo_remove_license($button) {
	...
	modal_content += '<br><br>' + wp.i18n.__( 'You can find your license data in the verified Stripe checkout email. If it is unavailable, open the license recovery help before removing these credentials.', 'ai-for-seo' );
...
function ai4seo_perform_remove_license($button) {
	...
	// Keep replacement credentials masked by default while retaining the existing reveal control.
	ai4seo_enable_license_key_entry_mode( $license_key_input_wrapper );
...
function ai4seo_accept_tos(reload_page = true) {
	...
	// Existing consent intentionally omits this optional checkbox; the server preserves that stored choice.
	const accepted_enhanced_reporting = ai4seo_exists_$( $accept_enhanced_reporting_checkbox )
		&& $accept_enhanced_reporting_checkbox.prop( 'checked' ) === true;
...
function ai4seo_sync_alt_text_injection_settings_visibility() {
...
function ai4seo_init_alt_text_injection_settings() {
	...
	$alt_text_injection_setting_toggle.on( 'change.ai4seo-alt-text-injection-toggle', ai4seo_sync_alt_text_injection_settings_visibility );

	const $advanced_settings_toggle_button = ai4seo_normalize_$( '#ai4seo-toggle-advanced-settings-button' );

	// Advanced visibility settles after its section animation, so defer the dependent setting refresh as before.
	const ai4seo_deferred_js_alt_text_injection_toggle = function () {
		setTimeout( ai4seo_sync_alt_text_injection_settings_visibility, 50 );
	};

	if (ai4seo_exists_$( $advanced_settings_toggle_button )) {
		$advanced_settings_toggle_button.off( 'click.ai4seo-alt-injection' );
		$advanced_settings_toggle_button.on( 'click.ai4seo-alt-injection', ai4seo_deferred_js_alt_text_injection_toggle );
	}

	ai4seo_sync_alt_text_injection_settings_visibility();
}

// =========================================================================================== \\

/**
 * Synchronize the persistent Advanced Settings disclosure with the current view state.
 *
 * @param {boolean} are_advanced_settings_shown Whether advanced settings are visible.
 */
function ai4seo_update_advanced_settings_toggle_button_state(are_advanced_settings_shown) {
	const $toggle_button = ai4seo_normalize_$( '#ai4seo-toggle-advanced-settings-button' );

	if (!ai4seo_exists_$( $toggle_button )) {
		return;
	}

	const label_attribute_name = are_advanced_settings_shown ? 'data-ai4seo-hide-label' : 'data-ai4seo-show-label';
	const fallback_label = are_advanced_settings_shown
		? wp.i18n.__( 'Hide Advanced Settings', 'ai-for-seo' )
		: wp.i18n.__( 'Show Advanced Settings', 'ai-for-seo' );
	const button_label = String( $toggle_button.attr( label_attribute_name ) || fallback_label );

	// One persistent control keeps focus while its text, icon, and expanded state move together.
	$toggle_button.attr( 'aria-expanded', are_advanced_settings_shown ? 'true' : 'false' );
	$toggle_button.attr( 'title', button_label );
	$toggle_button.toggleClass( 'ai4seo-advanced-settings-highlight', are_advanced_settings_shown );
	$toggle_button.find( '.ai4seo-advanced-settings-toggle-label' ).text( button_label );
	$toggle_button.find( '.ai4seo-advanced-settings-show-icon' ).toggleClass( 'ai4seo-display-none', are_advanced_settings_shown );
	$toggle_button.find( '.ai4seo-advanced-settings-hide-icon' ).toggleClass( 'ai4seo-display-none', !are_advanced_settings_shown );
}

// =========================================================================================== \\

/**
 * Initialize the persistent Advanced Settings disclosure from the saved form value.
 */
function ai4seo_init_advanced_settings() {
	...
	// Rebinding supports repeated page initialization without stacking disclosure handlers.
	if (ai4seo_exists_$( $advanced_settings_toggle_button )) {
		$advanced_settings_toggle_button.off( 'click.ai4seo-advanced-settings' );
		$advanced_settings_toggle_button.on(
			'click.ai4seo-advanced-settings',
			function () {
				if ($advanced_setting_state.val() === 'show') {
					ai4seo_hide_advanced_settings( true );
					return;
				}
...
/**
 * Show Advanced Settings and optionally animate the surrounding standard sections.
 *
 * @param {boolean} [show_fade_animation=false] Whether the user-triggered transition should animate.
 */
function ai4seo_show_advanced_settings(show_fade_animation = false) {
	// Show advanced settings and update the persistent disclosure in place.
	const $advanced_settings = ai4seo_normalize_$( '.ai4seo-is-advanced-setting' );
	const $advanced_setting_state = ai4seo_normalize_$( '#ai4seo-advanced-setting-state' );

	if (!ai4seo_exists_$( $advanced_settings ) || !ai4seo_exists_$( $advanced_setting_state )) {
		console.error( ai4seo_get_plugin_name() + ': Advanced settings state or controls missing in ai4seo_show_advanced_settings() — cannot reveal advanced options.' );
		return;
	}

	$advanced_settings.removeClass( 'ai4seo-display-none' ).show().css( 'display', '' );
	$advanced_setting_state.val( 'show' );
	ai4seo_update_advanced_settings_toggle_button_state( true );
	ai4seo_sync_alt_text_injection_settings_visibility();
	...
	if (show_fade_animation) {
		// Only a value that differs from the saved baseline needs a persistence reminder.
		if (ai4seo_refresh_unsaved_changes_state( $advanced_setting_state )) {
...
/**
 * Hide Advanced Settings and optionally animate the surrounding standard sections.
 *
 * @param {boolean} [show_fade_animation=false] Whether the user-triggered transition should animate.
 */
function ai4seo_hide_advanced_settings(show_fade_animation = false) {
	// Hide advanced settings and update the persistent disclosure in place.
	const $advanced_settings = ai4seo_normalize_$( '.ai4seo-is-advanced-setting' );
	const $advanced_setting_state = ai4seo_normalize_$( '#ai4seo-advanced-setting-state' );

	if (!ai4seo_exists_$( $advanced_settings ) || !ai4seo_exists_$( $advanced_setting_state )) {
		console.warn( ai4seo_get_plugin_name() + ': Advanced settings state or controls missing in ai4seo_hide_advanced_settings() — cannot conceal advanced options.' );
		return;
	}

	$advanced_settings.hide().addClass( 'ai4seo-display-none' ).css( 'display', '' );
	$advanced_setting_state.val( 'hide' );
	ai4seo_update_advanced_settings_toggle_button_state( false );
	ai4seo_sync_alt_text_injection_settings_visibility();
...
function ai4seo_validate_settings_inputs(input_values) {
	...
	return ai4seo_validate_custom_instruction_input_values( input_values, 3316141025 );
}
...
/**
 * Apply an element's logical disabled state without releasing an active page lock.
 *
 * @param {*} $element
 * @param {boolean} is_disabled
 */
function ai4seo_set_disabled_state_respecting_temporary_lock($element, is_disabled) {
	$element = ai4seo_normalize_$( $element );

	if (!ai4seo_exists_$( $element )) {
		return;
	}

	const temporary_lock_state_data_key = 'ai4seo-temporary-lock-state';
	const temporary_lock_state = $element.data( temporary_lock_state_data_key );

	if ($element.hasClass( 'ai4seo-temporary-locked' ) && temporary_lock_state && typeof temporary_lock_state === 'object') {
		temporary_lock_state.disabled = !!is_disabled;
		$element.data( temporary_lock_state_data_key, temporary_lock_state );
		return;
	}
...
function ai4seo_lock_and_disable_lockable_input_fields() {
	...
	$all_input_fields.each(
		function () {
			const $this_input_field = ai4seo_normalize_$( this );

			if (!ai4seo_exists_$( $this_input_field )) {
				return;
			}

			// Retain the first logical state if a second request encounters an already locked control.
			if (!$this_input_field.hasClass( 'ai4seo-temporary-locked' )) {
				$this_input_field.data(
					temporary_lock_state_data_key,
					{
						disabled: $this_input_field.prop( 'disabled' ) === true,
						had_inert: typeof $this_input_field.attr( 'inert' ) !== 'undefined',
						aria_disabled: typeof $this_input_field.attr( 'aria-disabled' ) === 'undefined'
							? null
							: $this_input_field.attr( 'aria-disabled' ),
					}
				);
			}

			$this_input_field.addClass( 'ai4seo-temporary-locked' );
			$this_input_field.prop( 'disabled', true );

			// Anchors do not support disabled, so make locked links non-interactive and expose their state.
			if ($this_input_field.is( 'a' )) {
				$this_input_field.attr( 'inert', 'inert' ).attr( 'aria-disabled', 'true' );
			}
		}
	);
}
...
function ai4seo_unlock_and_enable_lockable_input_fields() {
	...
	$all_input_fields.each(
		function () {
			const $this_input_field = ai4seo_normalize_$( this );
			const temporary_lock_state = $this_input_field.data( temporary_lock_state_data_key );

			if (!ai4seo_exists_$( $this_input_field )) {
				return;
			}

			$this_input_field.removeClass( 'ai4seo-temporary-locked' );
			$this_input_field.prop( 'disabled', !!temporary_lock_state?.disabled );

			if ($this_input_field.is( 'a' )) {
				if (temporary_lock_state?.had_inert) {
					$this_input_field.attr( 'inert', 'inert' );
				} else {
					$this_input_field.removeAttr( 'inert' );
				}

				if (temporary_lock_state?.aria_disabled === null || typeof temporary_lock_state?.aria_disabled === 'undefined') {
					$this_input_field.removeAttr( 'aria-disabled' );
				} else {
					$this_input_field.attr( 'aria-disabled', temporary_lock_state.aria_disabled );
				}
			}

			$this_input_field.removeData( temporary_lock_state_data_key );
		}
	);
}
...
// Close only the synchronously reserved checkout window and tolerate browser isolation races.
function ai4seo_close_purchase_window(purchase_window) {
	if (!purchase_window) {
		return;
	}

	try {
		if (!purchase_window.closed) {
			purchase_window.close();
		}
	} catch (error) {
		// A browser extension may have already isolated the window; there is nothing left to clean up.
	}
}

// =========================================================================================== \\

// Prefer the reserved tab while retaining a same-tab fallback for blocked or isolated popups.
function ai4seo_navigate_to_purchase_url(purchase_window, purchase_url) {
	if (purchase_window) {
		try {
			if (!purchase_window.closed && purchase_window.location) {
				purchase_window.location.replace( purchase_url );
				return 'new-tab';
			}
		} catch (error) {
			// Close the reserved blank tab before falling back after a browser isolation failure.
			ai4seo_close_purchase_window( purchase_window );
		}
	}
...
// Reserve and detach the external checkout tab while the administrator's click gesture is active.
function ai4seo_open_purchase_window() {
	let purchase_window = null;

	try {
		purchase_window = window.open( 'about:blank', '_blank' );
	} catch (error) {
		return null;
	}
...
// Prepare first-purchase protection only after an explicit administrator subscription CTA.
function ai4seo_init_subscription_pricing($button) {
	$button = ai4seo_normalize_$( $button );

	if (!ai4seo_exists_$( $button )) {
		console.error( ai4seo_get_plugin_name() + ': element "$button" missing in ai4seo_init_subscription_pricing() — cannot initialize pricing.' );
		return Promise.reject( new Error( 'Missing subscription pricing button.' ) );
	}

	const purchase_window = ai4seo_open_purchase_window();

	ai4seo_add_loading_html_to_element( $button );
	ai4seo_lock_and_disable_lockable_input_fields();
	ai4seo_show_loading_toast( wp.i18n.__( 'Preparing secure purchase options...', 'ai-for-seo' ) );

	return ai4seo_perform_ajax_call( 'ai4seo_init_subscription_pricing' )
		.then(
			response => {
				if (!response || typeof response.purchase_url !== 'string' || !response.purchase_url) {
					throw new Error( wp.i18n.__( 'The purchase destination was missing from the response.', 'ai-for-seo' ) );
				}

				ai4seo_show_success_toast( wp.i18n.__( 'Opening purchase options...', 'ai-for-seo' ) );
				ai4seo_set_unsaved_changes_state( $button, false );

				return ai4seo_navigate_to_purchase_url( purchase_window, response.purchase_url );
			}
		)
		.catch(
			error => {
				ai4seo_close_purchase_window( purchase_window );
				ai4seo_show_error_toast( 1513181226, error );
				return false;
			}
		)
		.finally(
...
// Keep the selected Credits Pack card, radio input, and price estimates synchronized.
...
// Initialize checkout only after a valid selection and always release the shared form lock.
function ai4seo_handle_select_credits_pack($submit_button) {
	...
	const selected_stripe_price_id = ai4seo_exists_$( $selected_credits_pack )
		? ai4seo_get_input_value( $selected_credits_pack )
		: '';

	if (!selected_stripe_price_id) {
	...
	// Keep the user informed while the server persists secure return and rotation state.
	ai4seo_show_loading_toast( wp.i18n.__( 'Initiating purchase...', 'ai-for-seo' ) );

	return ai4seo_perform_ajax_call( 'ai4seo_init_purchase', {stripe_price_id: selected_stripe_price_id} )
		.then(
			response => {
				...
				// Use the same tab for credit packs so browser history retains the local modal context.
...
function ai4seo_handle_payg_form_change() {
	const $payg_unavailable = ai4seo_normalize_$( '#ai4seo-payg-unavailable' );

	if (ai4seo_exists_$( $payg_unavailable )) {
		return;
	}
 ...
 */
function ai4seo_execute_import_settings($import_button, _new_settings, _categories) {
...
function ai4seo_update_dashboard_content(new_html) {
	...
	try {
		...
		// The catch may run before or after the parsed root is assigned.
		// noinspection ConstantConditionalExpressionJS
		if (!new_refresh_root_element || !replacement_target_element || !replacement_target_element.parentNode) {
			return false;
		}

		// The guard above establishes this even though catch-flow analysis loses the assignment.
...
function ai4seo_sync_child_nodes(old_container_element, new_container_element) {
	...
	function ai4seo_get_notice_index(node) {
		...
		return node.getAttribute( 'data-notification-index' ) || '';
	}
