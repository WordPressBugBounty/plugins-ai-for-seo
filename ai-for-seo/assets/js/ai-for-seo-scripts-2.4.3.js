...
// Keep one observer per AIOSEO editor instance.
const ai4seo_aioseo_editor_observers = new WeakMap();
...
const ai4seo_seopress_editor_observers = new WeakMap();
...
// Keep one observer per Slim SEO React editor instance.
const ai4seo_slim_seo_editor_observers = new WeakMap();
...
// One document observer discovers all supported JavaScript-rendered SEO editor roots.
let ai4seo_generation_document_observer = null;
...
const ai4seo_js_file_path = ai4seo_get_localization_parameter( 'ai4seo_admin_script_url' ) + '?ver=' + encodeURIComponent( ai4seo_admin_scripts_version_number );
const ai4seo_js_file_id = 'ai-for-seo-scripts-js';

const ai4seo_css_file_path = ai4seo_get_localization_parameter( 'ai4seo_admin_style_url' ) + '?ver=' + encodeURIComponent( ai4seo_admin_scripts_version_number );
...
if (typeof jQuery === 'function') {
	...
	jQuery( document ).ready(
		...
		// The current assets have consumed the one-time update token, so keep future visits on plugin-version URLs.
		...
		// Activate the Help section referenced by the location hash.
...
function ai4seo_schedule_wordpress_media_generate_buttons_init() {
	...
	ai4seo_wordpress_media_generate_buttons_init_timer = setTimeout(
        function () {
...
function ai4seo_init_media_frame_open_event() {
	...
	if (
		...
		typeof window.wp.media.view.MediaFrame.Select === 'undefined'
	) {
		return;
	}

	const prototype = wp.media.view.MediaFrame.Select.prototype;

	// Prevent double override.
	if (prototype.ai4seo_open_overridden === true) {
		return;
	}

	const native_open = prototype.open;

	// Basic debounce to prevent multiple rapid executions.
	let ai4seo_last_open_timestamp = 0;
	...
	// Mark as overridden.
	prototype.ai4seo_open_overridden = true;
}
...
function ai4seo_init_load_scripts_click_listeners() {
	...
	if (!ai4seo_exists_$( $parent_document_body )) {
		console.warn( ai4seo_get_plugin_name() + ': element \"$parent_document_body\" missing in document_ready_handler() \u2014 cannot attach delegated bindings.' );
		return;
	}
	...
	for (let i = 0; i < external_scripts_click_selectors.length; i++) {
		...
		$parent_document_body.on(
            'click.ai4seo-init-scripts',
            external_scripts_click_selector,
            function () {
			setTimeout(
                function () {
				// Call function to load js-file to main-window.
				ai4seo_try_load_js_file_to_top_document(
                    ai4seo_js_file_path,
                    ai4seo_js_file_id,
                    function () {
				...
				ai4seo_try_load_css_file_to_top_document( ai4seo_css_file_path, ai4seo_css_file_id );
...
function ai4seo_try_load_js_file_to_top_document(url, script_id = '', callback = null) {
	...
	if (ai4seo_exists_$( '#' + script_id, window.top.document )) {
		return;
	}
	...
	if (!ai4seo_exists_$( $script )) {
		console.error( 'Could not create script element in ai4seo_load_js_file() \u2014 aborting script load.' ); // attention: no plugin name here as this error may happen before localization is set.
		return;
	}
	...
	if (!ai4seo_exists_$( $window_top_head )) {
		console.error( 'Parent window head element missing in ai4seo_load_js_file() \u2014 aborting script load.' ); // attention: no plugin name here as this error may happen before localization is set.
		return;
	}
...
function ai4seo_try_load_css_file_to_top_document(url, script_id = '', callback = null) {
	// Stop script if no url is given.
	if (!url) {
		return;
	}
	...
	if (!ai4seo_exists_$( $link )) {
		console.error( ai4seo_get_plugin_name() + ': Could not create link element in ai4seo_load_css_file() \u2014 aborting stylesheet load.' );
		return;
	}
	...
	if (!ai4seo_exists_$( $window_top_head )) {
		console.error( ai4seo_get_plugin_name() + ': Parent window head element missing in ai4seo_load_css_file() \u2014 aborting stylesheet load.' );
		return;
	}
...
function ai4seo_init_html_elements() {
	...
	// Show a third-party synchronization warning carried across the preceding save reload.
	...
	// Prepare AIOSEO's lazy social editors and the always-mounted Generate All field proxies.
	ai4seo_init_aioseo_generation_ui();

	// Prepare SEOPress's lazy title/social tabs and the always-mounted Generate All field proxies.
	ai4seo_init_seopress_generation_ui();

	// Prepare Slim SEO's React-rendered title and description fields.
	ai4seo_init_slim_seo_generation_ui();

	// Prepare Squirrly's AJAX-rendered search and social metadata fields.
	ai4seo_init_squirrly_generation_ui();
	...
	// Keep the metadata-editor focus-keyphrase warning in sync with generated and manually edited values.
	ai4seo_init_metadata_editor_focus_keyphrase_warning();
	...
	ai4seo_init_generate_all_buttons();
	...
	// init plugin deactivation feedback interception.
 ...
 */
function ai4seo_init_unsaved_changes_warnings() {
	const $unsaved_changes_containers = ai4seo_normalize_$( '.ai4seo-unsaved-changes-warnings' );
...
function ai4seo_init_buttons() {
	...
	// Global release (mouseup may happen outside the button).
	$window.off(
	...
	$window.on(
		...
		function (event) {
			if (!ai4seo_exists_$( ai4seo_last_pressed_button )) {
				// ai4seo_console_debug(ai4seo_get_plugin_name() + ': Button release detected but no active button stored.', event.type);.
...
function ai4seo_schedule_gutenberg_editor_generate_buttons_init() {
	...
	ai4seo_init_gutenberg_editor_generate_buttons_init_timeout = setTimeout(
        function () {
...
function ai4seo_init_gutenberg_editor_generate_buttons() {
	...
	$wp_block_media_text_media.on(
        'click.ai4seo-gutenberg',
        function () {
	...
	ai4seo_init_gutenberg_editor_loop_timeout = setTimeout(
        function () {
		ai4seo_init_gutenberg_editor_generate_buttons();
        },
        2500
...
function ai4seo_get_gutenberg_editor_$() {
	...
	if (!ai4seo_exists_$( $gutenberg_editor_iframe_document )) {
		console.error( ai4seo_get_plugin_name() + ': Gutenberg iframe document missing in ai4seo_init_gutenberg_editor_generate_buttons() \u2014 deferring block integrations.' );
		return null;
	}
...
function ai4seo_get_gutenberg_editor_iframe_element() {
	...
	if (!ai4seo_exists_$( 'iframe[name="editor-canvas"]' )) {
		// ai4seo_console_debug(ai4seo_get_plugin_name() + ': selector iframe[name=\"editor-canvas\"] missing in ai4seo_get_gutenberg_editor_iframe_element() \u2014 skipping editor access.');.
		return null;
	}
...
function ai4seo_open_get_more_credits_modal() {
	...
	if (!ai4seo_exists_$( $all_items )) {
		console.error( ai4seo_get_plugin_name() + ': elements \"$all_items\" missing in ai4seo_open_get_more_credits_modal() \u2014 credits carousel animation skipped.' );
		return;
	}
	...
	$all_items.each(
		...
		if (!ai4seo_exists_$( $item )) {
			console.error( ai4seo_get_plugin_name() + ': element \"$item\" missing in ai4seo_open_get_more_credits_modal() \u2014 skipping iteration.' );
			return;
		}
...
function ai4seo_try_init_generate_buttons_x_times(times = 10, frequency = 1000) {
...
/**
 * Watches for supported JavaScript-rendered SEO editor roots mounted after page load.
 */
function ai4seo_init_generation_document_observer() {
	if (ai4seo_generation_document_observer !== null || typeof MutationObserver === 'undefined' || !document.body) {
		return;
	}

	// Each root retains its source-specific rescan while sharing the document mutation pass.
	const generation_editor_details = [
		{
	...
	ai4seo_generation_document_observer = new MutationObserver(
		function (mutations) {
			const mounted_generation_roots = new Map();

			// One mutation can contain a root itself or a parent inserted around several roots.
			for (const mutation of mutations) {
				for (const added_node of Array.from( mutation.addedNodes || [] )) {
					...
					for (const editor_details of generation_editor_details) {
						if (added_node.matches( editor_details.root_selector )) {
							mounted_generation_roots.set( added_node, editor_details.rescan_generation_ui );
						}

						for (const nested_generation_root of added_node.querySelectorAll( editor_details.root_selector )) {
							mounted_generation_roots.set( nested_generation_root, editor_details.rescan_generation_ui );
						}
					}
				}
			}
			...
			for (const [mounted_generation_root, rescan_generation_ui] of mounted_generation_roots) {
				window.setTimeout( () => rescan_generation_ui( mounted_generation_root ), 0 );
			}
		}
	);
	ai4seo_generation_document_observer.observe( document.body, {childList: true, subtree: true} );
}
 ...
 * @param {WeakMap} editor_observers Per-editor observer registry.
 ...
 */
function ai4seo_observe_generation_editor_fields(editor_root, editor_observers, mounted_field_selector, rescan_generation_ui) {
	if (typeof MutationObserver === 'undefined'
		|| !editor_root
		|| editor_observers.has( editor_root )
		|| !mounted_field_selector
		|| typeof rescan_generation_ui !== 'function') {
		return;
	}

	// Debouncing collapses framework subtree batches into one control reinitialization.
	const debounced_rescan_generation_ui = ai4seo_debounce( rescan_generation_ui, 50 );
	const generation_editor_observer = new MutationObserver(
		function (mutations) {
			...
			if (editor_field_was_mounted) {
				debounced_rescan_generation_ui();
			}
		}
	);

	generation_editor_observer.observe( editor_root, {childList: true, subtree: true} );
	editor_observers.set( editor_root, generation_editor_observer );
}
...
function ai4seo_rescan_slim_seo_generation_ui(slim_seo_root) {
	...
	if (!ai4seo_exists_$( $slim_seo_metabox )) {
		return;
	}
...
function ai4seo_init_slim_seo_generation_ui(slim_seo_root = null) {
	...
	ai4seo_init_generation_document_observer();
	...
	// Derive the mutation selector from the same field registry used by the generation controls.
	...
	// Observe each editor independently because Slim SEO can replace its controlled fields after mount.
	$slim_seo_roots.each(
		function () {
			const this_slim_seo_root = this;

			ai4seo_observe_generation_editor_fields(
				this_slim_seo_root,
				ai4seo_slim_seo_editor_observers,
				mounted_field_selector,
				() => ai4seo_rescan_slim_seo_generation_ui( this_slim_seo_root )
			);
			ai4seo_rescan_slim_seo_generation_ui( this_slim_seo_root );
		}
	);
...
function ai4seo_init_one_aioseo_generation_root(aioseo_root) {
	...
	ai4seo_observe_generation_editor_fields(
		aioseo_root_element,
		ai4seo_aioseo_editor_observers,
		'.ql-editor',
...
function ai4seo_init_one_seopress_generation_root(seopress_root) {
	...
	ai4seo_observe_generation_editor_fields(
		...
		ai4seo_seopress_editor_observers,
		mounted_field_selector,
 ...
 */
function ai4seo_init_generate_buttons(scope = null) {
	...
	jQuery.each(
		...
		function (this_generate_data_for_input_selector, this_generate_data_for_input_details) {
			...
			const $this_generate_data_for_inputs = ai4seo_normalize_$( this_generate_data_for_input_selector, scope );
...
/**
 * Bind the focus-keyphrase warning to the current values in each metadata editor modal.
 *
 * The warning markup is always rendered by PHP so it can react without reloading the modal.
 */
function ai4seo_init_metadata_editor_focus_keyphrase_warning() {
	// AJAX modal refreshes can replace the editor markup, so resolve all current modal instances on every initialization.
	const $metadata_editor_modals = ai4seo_normalize_$( '#ai4seo-metadata-editor-modal' );

	if (!ai4seo_exists_$( $metadata_editor_modals )) {
		return;
	}

	// Bind each modal independently so its warning only reflects values from the same editor instance.
	$metadata_editor_modals.each(
        function () {
		const $metadata_editor_modal = ai4seo_normalize_$( this );
		const $related_metadata_inputs = $metadata_editor_modal.find(
			'#ai4seo_metadata_focus-keyphrase, #ai4seo_metadata_meta-title, #ai4seo_metadata_meta-description'
		);

		if (!ai4seo_exists_$( $related_metadata_inputs )) {
			return;
		}

		// Namespace and replace the listeners because generic modal initialization may run more than once.
		$related_metadata_inputs
			.off( 'input.ai4seo-focus-keyphrase-warning change.ai4seo-focus-keyphrase-warning' )
			.on(
                'input.ai4seo-focus-keyphrase-warning change.ai4seo-focus-keyphrase-warning',
                function () {
				ai4seo_refresh_metadata_editor_focus_keyphrase_warning( $metadata_editor_modal );
                }
            );
...
/**
 * Check whether one metadata-editor field contains a non-whitespace value.
 *
 * This uses the shared input reader so supported input types keep the same value semantics as other plugin features.
 *
 * @param {jQuery} $metadata_editor_modal Metadata editor containing the field.
 * @param {string} field_selector Selector for one field inside the editor.
 *
 * @return {boolean} Whether the field contains a value.
 */
function ai4seo_metadata_editor_field_has_value($metadata_editor_modal, field_selector) {
	// Accept the same jQuery-compatible input shapes as the surrounding editor helpers.
	$metadata_editor_modal = ai4seo_normalize_$( $metadata_editor_modal );

	if (!ai4seo_exists_$( $metadata_editor_modal )) {
		return false;
	}

	// Use only the first match because metadata identifiers are unique inside one modal.
	const $field = $metadata_editor_modal.find( field_selector ).first();

	return ai4seo_exists_$( $field )
		&& String( ai4seo_get_input_value( $field ) || '' ).trim() !== '';
}

// =========================================================================================== \\

/**
 * Show the focus-keyphrase warning only while related metadata exists without a keyphrase.
 *
 * @param {jQuery|HTMLElement|string} scope Metadata editor modal or a descendant of it.
 */
function ai4seo_refresh_metadata_editor_focus_keyphrase_warning(scope) {
	// Resolve descendant scopes back to their owning modal so generation callbacks can pass their closest container.
	const $scope = ai4seo_normalize_$( scope );
	const $metadata_editor_modal = $scope.is( '#ai4seo-metadata-editor-modal' )
		? $scope.first()
		: $scope.closest( '#ai4seo-metadata-editor-modal' ).first();

	if (!ai4seo_exists_$( $metadata_editor_modal )) {
		return;
	}

	// Anchor the warning lookup to the focus-keyphrase form item so unrelated warnings remain untouched.
	const $focus_keyphrase = $metadata_editor_modal.find( '#ai4seo_metadata_focus-keyphrase' ).first();

	if (!ai4seo_exists_$( $focus_keyphrase )) {
		return;
	}

	// Resolve the message inside that form item because other metadata fields can render warnings of their own.
	const $warning = $focus_keyphrase
		.closest( '.ai4seo-form-item' )
		.find( '.ai4seo-editor-field-warning-message' )
		.first();

	if (!ai4seo_exists_$( $warning )) {
		return;
	}

	// Warn only when existing metadata would otherwise be regenerated without a focus keyphrase.
	const should_show_warning = !ai4seo_metadata_editor_field_has_value( $metadata_editor_modal, '#ai4seo_metadata_focus-keyphrase' )
		&& (
			ai4seo_metadata_editor_field_has_value( $metadata_editor_modal, '#ai4seo_metadata_meta-title' )
			|| ai4seo_metadata_editor_field_has_value( $metadata_editor_modal, '#ai4seo_metadata_meta-description' )
		);

	$warning.toggleClass( 'ai4seo-display-none', !should_show_warning );
}
 ...
 */
function ai4seo_init_metadata_editor_length_feedback() {
	// AJAX modal refreshes can replace the editor markup, so bind the current fields on every initialization.
	const $length_tracked_inputs = ai4seo_normalize_$( '.ai4seo-editor-length-tracked' );
...
/**
 * Refresh character counts and target-range states within one field or container.
 *
...
function ai4seo_refresh_metadata_editor_length_feedback(scope) {
	...
	const $length_tracked_inputs = $scope.is( '.ai4seo-editor-length-tracked' )
		? $scope
	...
	$length_tracked_inputs.each(
		function () {
			// Resolve each field's feedback element and server-provided target before updating its state.
			const $input = ai4seo_normalize_$( this );
			const $length_feedback = $input.closest( '.ai4seo-form-item' ).find( '.ai4seo-editor-length-feedback' ).first();
			const minimum_length = Number.parseInt( String( $input.attr( 'data-ai4seo-min-length' ) || '' ), 10 );
			const maximum_length = Number.parseInt( String( $input.attr( 'data-ai4seo-max-length' ) || '' ), 10 );

			// Ignore incomplete editor markup rather than applying a misleading partial target state.
			if (!ai4seo_exists_$( $length_feedback )
				|| !Number.isInteger( minimum_length )
				|| !Number.isInteger( maximum_length )
				|| minimum_length < 1
				|| maximum_length < minimum_length) {
				return;
			}

			// Reuse the shared Unicode counter so metadata and custom-instruction limits match PHP mb_strlen().
			const input_length = ai4seo_get_text_length( ai4seo_get_input_value( $input ) );
			/* translators: 1: Current character count. 2: Minimum target length. 3: Maximum target length. */
			const feedback_template = wp.i18n._n(
				'%1$d character · target %2$d–%3$d',
				'%1$d characters · target %2$d–%3$d',
				input_length,
				'ai-for-seo'
			);
			// Empty fields remain neutral because the target describes generated-text quality, not required input.
			const is_outside_target = 0 < input_length
				&& (input_length < minimum_length || input_length > maximum_length);

			// Update text and state together so assistive and visual feedback always describe the same count.
			$length_feedback
				.text(
					wp.i18n.sprintf(
						feedback_template,
				...
				.toggleClass( 'ai4seo-editor-length-feedback-outside-target', is_outside_target );
		}
	);
}
...
function ai4seo_init_auto_resize_textareas() {
	...
	if (!ai4seo_exists_$( $textareas )) {
		// ai4seo_console_debug(ai4seo_get_plugin_name() + ': elements \".ai4seo-auto-resize-textarea\" missing in ai4seo_init_auto_resize_textareas() \u2014 skipping auto-resize binding.');.
		return;
	}

	// ai4seo_console_debug(ai4seo_get_plugin_name() + ': Initializing auto-resize for ' + $textareas.length + ' textareas in ai4seo_init_auto_resize_textareas().');.
	...
	$textareas.each(
		...
		if (!ai4seo_exists_$( $this_textarea )) {
			console.error( ai4seo_get_plugin_name() + ': element \"$this_textarea\" missing in ai4seo_init_auto_resize_textareas() \u2014 skipping item.' );
			return;
		}
...
function ai4seo_get_custom_instruction_counter_for_input($input) {
	...
	return ai4seo_normalize_$( '.ai4seo-custom-instructions-counter' ).filter(
        function () {
...
function ai4seo_init_tooltips() {
	...
	const $tooltip_holders = ai4seo_normalize_$( '.ai4seo-tooltip-holder' );
	const $tooltip_triggers = ai4seo_normalize_$( '.ai4seo-tooltip-trigger' );
	const $tooltips = ai4seo_normalize_$( '.ai4seo-tooltip' );

	// Remove the delegated close handler before current tooltip collections are rebound below.
	...
	if (ai4seo_exists_$( $tooltip_holders )) {
		...
		$tooltip_holders.each(
			function () {
				...
				if (!tooltip_id) {
					...
					} while (document.getElementById( tooltip_id ));
		...
		$tooltip_holders.on(
            ...
            function (event) {
			const $this_tooltip_child = jQuery( this ).find( '.ai4seo-tooltip' );
		...
		$tooltip_holders.on(
            ...
            function () {
			const $this_tooltip = jQuery( this ).find( '.ai4seo-tooltip' );
			...
			ai4seo_hide_tooltip( $this_tooltip, 200 );
            }
        );
		...
		$tooltip_holders.on(
			...
			const should_show_tooltip = !$this_tooltip_child.is( ':visible' );

			// Resolve tooltips at click time so newly inserted tooltips are included in the close behavior.
			const $current_tooltips = ai4seo_normalize_$( '.ai4seo-tooltip' );

			if (ai4seo_exists_$( $current_tooltips )) {
				ai4seo_hide_tooltip( $current_tooltips, 0 ); // Hide other tooltips and synchronize their ARIA state.
			}
	...
	if (ai4seo_exists_$( $tooltip_triggers )) {
		...
		$tooltip_triggers.on(
			...
			function (event) {
				...
				event.stopPropagation();
				ai4seo_hide_tooltip( jQuery( this ).closest( '.ai4seo-tooltip-holder' ).children( '.ai4seo-tooltip' ).first() );
			}
		);
		...
		$tooltip_triggers.on(
			...
			function () {
				ai4seo_hide_tooltip( jQuery( this ).closest( '.ai4seo-tooltip-holder' ).children( '.ai4seo-tooltip' ).first(), 200 );
			}
		);
	}
	...
	if (ai4seo_exists_$( $tooltips )) {
		...
		$tooltips.on(
			...
			setTimeout(
				...
				if (ai4seo_exists_$( $current_tooltips )) {
					ai4seo_hide_tooltip( $current_tooltips, 0 ); // Hide all tooltips and synchronize their ARIA state.
				}
                },
                2
            );
...
function ai4seo_init_select_all_checkboxes() {
	...
	$select_all_checkboxes.each(
		...
		const $all_target_checkboxes = ai4seo_normalize_$( target_checkbox_selector + ':not(:disabled)' );
		const $all_target_checkboxes_including_disabled = ai4seo_normalize_$( target_checkbox_selector );
...
function ai4seo_show_tooltip($tooltip, event) {
	...
	if (!has_pointer_coordinates) {
		const $tooltip_trigger = $tooltip.closest( '.ai4seo-tooltip-holder' ).children( '.ai4seo-tooltip-trigger' ).first();
		const $tooltip_anchor = ai4seo_exists_$( $tooltip_trigger ) ? $tooltip_trigger : $tooltip.closest( '.ai4seo-tooltip-holder' );
	...
	if (relative_interaction_y <= vertical_buffer_zone + tooltip_height) {
	...
	$tooltip.closest( '.ai4seo-tooltip-holder' ).children( '.ai4seo-tooltip-trigger' ).first().attr( 'aria-expanded', 'true' );
	$tooltip.fadeIn( 100 );
}
...
function ai4seo_hide_tooltip($tooltip, fade_duration = 100) {
	...
	$tooltip.each(
		function () {
			...
			$this_tooltip.closest( '.ai4seo-tooltip-holder' ).children( '.ai4seo-tooltip-trigger' ).first().attr( 'aria-expanded', 'false' );
			$this_tooltip.stop( true, true );

			// A zero duration is used by close-all paths that must finish before another tooltip opens.
			if (fade_duration > 0) {
				$this_tooltip.fadeOut( fade_duration );
			} else {
				$this_tooltip.hide();
			}
		}
...
function ai4seo_normalize_$(mixed = null, scope = null) {
	...
	// NO CONTEXT: TRY WHOLE DOCUMENT + PARENT.DOCUMENT AS A FALLBACK.
	if (scope === null) {
		try {
			$mixed = jQuery( mixed );
		} catch (error) {
		...
		// fallback: try window.parent.document context.
		try {
			$mixed = jQuery( mixed, window.parent.document );
		} catch (error) {
			console.error( ai4seo_get_plugin_name() + ': Error normalizing jQuery element with window.parent.document context in ai4seo_normalize_$(): ' + error );
			return jQuery(); // return empty jQuery object on error.
		}

		// if we found a jQuery object with the selector, then return it.
		if (typeof $mixed.length !== 'undefined' && $mixed.length > 0) {
			return $mixed;
		}

		return jQuery(); // return empty jQuery object if no matches found.
	}

	// WITH CONTEXT
	// normalize context to jQuery in current document first.
	try {
		$scope = jQuery( scope );
	} catch (error) {
		console.error( ai4seo_get_plugin_name() + ': Error normalizing context in ai4seo_normalize_$() before applying parent fallback: ' + error );
		return jQuery();
	}

	// fallback: try context in window.parent.document too.
	if (typeof $scope.length === 'undefined' || $scope.length === 0) {
		try {
			$scope = jQuery( scope, window.parent.document );
		} catch (error) {
			console.error( ai4seo_get_plugin_name() + ': Error normalizing context with window.parent.document in ai4seo_normalize_$(): ' + error );
			return jQuery();
		}
	}

	if (typeof $scope.length === 'undefined' || $scope.length === 0) {
		// ai4seo_console_debug(ai4seo_get_plugin_name() + ': Context element not found in ai4seo_normalize_$(): ', context);.
		return jQuery();
	}
	...
	for (let i = 0; i < $scope.length; i++) {
		let $this_scope_item = jQuery( $scope[i] );

		try {
			// works well if $this_scope_item is a main DOM element (e.g. window.parent).
			$mixed = jQuery( mixed, $this_scope_item );

			// if we found a jQuery object with the selector, then add it.
			if (typeof $mixed.length !== 'undefined' && $mixed.length > 0) {
				$scoped_mixed = $scoped_mixed.add( $mixed );
			} else {
				$mixed = jQuery( mixed );

				// workaround for DOM elements that and the mixed and the scope are the same.
				if ($this_scope_item === $mixed || jQuery.contains( $this_scope_item, $mixed )) {
					$scoped_mixed = $scoped_mixed.add( $mixed );
				} else {
					// this route works best if scope is a jQuery object.
					$mixed = $this_scope_item.find( $mixed );
			...
			console.error( ai4seo_get_plugin_name() + ': Error normalizing jQuery element with context in ai4seo_normalize_$(): ' + error );
			return jQuery();
		}
	}

	if (typeof $scoped_mixed.length !== 'undefined' && $scoped_mixed.length > 0) {
		return $scoped_mixed;
	}
...
function ai4seo_generate_with_ai($generate_button, ajax_action, generate_data_for_input_instructions = [], post_id = false, overwrite_data = true, try_read_page_content_via_js = false) {
	...
	ai4seo_perform_ajax_call( ajax_action, ajax_data )
		.then(
			...
			// Generated metadata bypasses native input events, so explicitly synchronize the related warning.
			if (processing_context === 'metadata') {
				ai4seo_refresh_metadata_editor_focus_keyphrase_warning( $closest_container );
			...
			jQuery.each(
				...
				function (_, this_generate_data_for_selector) {
					...
					if (!ai4seo_exists_$( $this_possible_generate_button )) {
						console.warn( ai4seo_get_plugin_name() + ': element \"$this_possible_generate_button\" missing in ai4seo_generate_with_ai() \u2014 cannot add loading state to generate button for selector ' + this_generate_data_for_selector + '.' );
						return;
					}
		...
		function (this_generation_field_identifier, this_generate_data_for_input_instruction) {
			...
			if (!this_generated_data || this_generated_data.toString().trim() === '') {
				...
				jQuery.each(
					...
					function (_, this_generate_data_for_input_selector) {
						ai4seo_update_editor_field_generated_source_message( this_generate_data_for_input_selector, '', true, scope );
					}
				);
...
// Keep live source labels limited to SOOZ-owned editor modals; unresolved fields remove provenance without touching values.
function ai4seo_update_editor_field_generated_source_message(generate_data_for_input_selector, generated_at_output = '', remove_source_message = false, scope = null) {
	...
	if (remove_source_message) {
		$label.find( '.ai4seo-editor-field-source-message' ).first().remove();
		return;
	}
 ...
 * @param {jQuery|HTMLElement|null} scope Optional editor instance.
 */
function ai4seo_init_generate_all_buttons(scope = null) {
	// Check if current page is attachment-page
	...
	for (const this_processing_context in ai4seo_generate_all_button_selectors) {
		ai4seo_generate_all_button_selectors[this_processing_context].forEach(
			function (this_generate_all_button_selector) {
				...
				const $generate_all_button_containers = ai4seo_normalize_$( this_generate_all_button_selector, scope );
...
function ai4seo_add_generate_all_buttons(processing_context, $generate_all_buttons_container) {
	...
	jQuery.each(
		...
		let onclick = '';
		let button_title = '';
		let button_label = ai4seo_get_sooz_logo_svg_tag( 'sooz-oo' );
...
function ai4seo_open_notification_modal(headline = '', content = '', footer = '', modal_settings = {}) {
	...
	let default_footer = "<button type='button' onclick='ai4seo_close_modal(\"" + modal_id + "\");' class='ai4seo-button ai4seo-primary-button'>" + wp.i18n.__( 'Close', 'ai-for-seo' ) + '</button>';
...
function ai4seo_open_ajax_modal(ajax_action, ajax_data = {}, modal_settings = {}) {
	...
	ai4seo_perform_ajax_call( ajax_action, ajax_data, false )
		.then(
			...
			// Metadata editor fields can be hydrated by external editor integrations before generic modal init runs.
			// Synchronize this warning immediately after the AJAX response enters its final modal container.
			if (modal_id === 'ai4seo-metadata-editor-modal') {
				ai4seo_init_metadata_editor_focus_keyphrase_warning();
			}
...
function ai4seo_add_open_edit_metadata_modal_button_to_edit_page_header() {
	...
	if (!ai4seo_exists_$( $header_bar_buttons_container )) {
		// ai4seo_console_debug(ai4seo_get_plugin_name() + ': no interface pinned items found in ai4seo_add_open_edit_metadata_modal_button_to_edit_page_header() \u2014 cannot add toolbar button.');.
		return;
	}
...
function ai4seo_init_dashboard_progress_bar() {
	...
	// Create wrapper.
	const $new_dashboard_progress_wrapper = ai4seo_normalize_$(
        '<div>',
        {
	...
	// Create progress bar.
	const $new_dashboard_progress_bar = ai4seo_normalize_$(
        '<div>',
        {
	...
	const $new_dashboard_debug_counter = ai4seo_normalize_$(
        '<div>',
        {
